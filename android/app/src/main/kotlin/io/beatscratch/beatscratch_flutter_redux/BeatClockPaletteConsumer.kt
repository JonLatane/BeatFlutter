 ...
 */
object BeatClockPaletteConsumer : Patterns {
  var palette: Music.Score? = null
  set(value) {
    field = value
...
//    }
  ...
  private fun getNextSection(): Music.Section {
    ...
    loop@ for(candidate in palette!!.sectionsList) {
      when {
        candidate === currentSection -> isNextSection = true
        isNextSection                -> { nextSection = candidate; break@loop }
      }
    }
    return nextSection ?: palette!!.sectionsList.first()
  }

  fun tick() {
    (palette to currentSection).letCheckNull { palette: Music.Score, section: Music.Section ->
