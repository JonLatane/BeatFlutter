...
//            sampler.startNote(36, withVelocity:90, onChannel:0)
            ...
            let args = [UInt8](data)
//            let args = [UInt8](_immutableCocoaArray: (call.arguments as! FlutterStandardTypedData).data)
            if((args[0] & 0xF0) == 0x90) {
              print("noteOn");
                conductor.playNote(note: args[1], velocity: args[2], channel: args[0] & 0xF)
//                synth.play(noteNumber: args[1], velocity: args[2], frequency: 0, channel: args[0] & 0xF)
            } else if((args[0] & 0xF0) == 0x80) {
              print("noteOff");
                conductor.stopNote(note: args[1], channel: args[0] & 0xF)
//                synth.stop(noteNumber: MIDINoteNumberargs[0])
//              MIDI.noteOff(args[0] & 0xF, args[1], 0);
            } else {
