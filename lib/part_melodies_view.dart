...
class PartMelodiesView extends StatefulWidget {
  ...
  PartMelodiesView({this.score, this.setState, this.axis = Axis.horizontal});
...
//      scrollDirection: widget.axis,
//      padding: EdgeInsets.all(0),
//      children: widget.score.parts.map(_buildPart).toList(),//[ for (Part part in widget.score.parts) _buildPart(part) ],
...
class _Part extends StatelessWidget {
  ...
  Part get part {
    return score.parts[partPosition];
  }
...
//    );
  ...
  Widget _buildView2(BuildContext context, ReorderableItemState state) {
    return Text(part.id);
  }

  Widget _buildView(BuildContext context, ReorderableItemState state) {
...
//        )
        ...
        : Row(children: [
            FlatButton(
              color: part.instrument.type == InstrumentType.drum ? Colors.brown : Colors.black,
              child:
                  DelayedReorderableListener(child: Text(part.instrument.name, style: TextStyle(color: Colors.white))),
              onPressed: () => {},
            )
          ]);
  }
  ...
  Widget _buildChild(BuildContext context, ReorderableItemState state) {
    ...
    Widget content = Container(
      ...
      child: Stack(children: [
        RaisedButton(child:Text("Melody ${melody.id}"), onPressed: () => {},)
      ])
//      child: SafeArea(
//          top: false,
//          bottom: false,
