...
*/
  ...
  _drawNotationMelody(Canvas canvas) {
    ...
    if (melody.id == "keyboardDummy" || melody.id == "colorboardDummy") {
      ...
      final ArgumentList key = ArgumentList([
        melody.id,
        otherMelodiesOnStaff.map((e) => e.id).join(),
        section.id,
        beatPosition
      ]);
      instructions =
          notationRenderingCache.putIfAbsent(key, calculateInstructions);
    }
  ...
  _renderNoteheadSignAndLedgers(
    ...
    instruction.ledgerLines.forEach((position) {
      canvas.drawLine(Offset(left - 3 * xScale, position * xScale),
  ...
  _planNoteheadsSignsAndLedgers(_RenderInstructions result) {
    ...
    double howFarIntoBeatAreWe =
        (elementPosition % melody.subdivisionsPerBeat) /
            melody.subdivisionsPerBeat;
    double xOffset = -35 * howFarIntoBeatAreWe;

    if (tones.isNotEmpty) {
      double noteheadWidth =
          18; //min(letterStepSize * 2, maxFittableNoteheadWidth);
      ...
      playbackNotes.forEach((note) {
        ...
        bool shouldStagger = playbackNotes.any((it) =>
            (it.diatonicValue % 2 == 0 &&
                (it.diatonicValue == note.diatonicValue + 1 ||
                    it.diatonicValue == note.diatonicValue - 1)) ||
            (it.diatonicValue == note.diatonicValue && it.tone > note.tone));
...
//        val previousSign = previousSignOf(melody, harmony, note, elementPosition)
        ...
        switch (note.sign) {
          ...
          case NoteSign.flat:
            ...
            break;
          case NoteSign.double_flat:
            if (previousSign != NoteSign.double_flat)
              signToDraw = NoteSign.double_flat;
            break;
          case NoteSign.double_sharp:
            if (previousSign != NoteSign.double_sharp)
              signToDraw = NoteSign.double_sharp;
            break;
          case NoteSign.natural:
          default:
      ...
      if (stemsUp) {
        double stemX = (hadStaggeredNotes)
            ...
            : bounds.left +
                1.83 * noteheadWidth; //bounds.right - 0.965 * noteheadWidth;
        double startY =
            maxCenter + noteheadHeight * ((maxWasStaggered) ? .1 : -.1);
        ...
        double stemX = (hadStaggeredNotes)
            ? bounds.left +
                0.887 * noteheadWidth //bounds.right - 0.95 * noteheadWidth
            : bounds.left +
                0.97 * noteheadWidth; //bounds.right - 1.837 * noteheadWidth;
        double startY = minCenter +
            noteheadHeight *
    ...
    return recentSignCache.putIfAbsent(args, () {
      return _calculateMostRecentSignsOf(
        ...
        relevantMelodies: relevantMelodies
            .where((m) => m.id != "keyboardDummy" && m.id != "colorboardDummy"),
      );
    });
  }
  ...
  Iterable<NoteSign> _calculateMostRecentSignsOf({
    ...
    relevantMelodies.forEach((relevantMelody) {
      int relevantMelodyIndex = elementPosition.convertPatternIndex(
          fromSubdivisionsPerBeat: melody.subdivisionsPerBeat,
          toSubdivisionsPerBeat: relevantMelody.subdivisionsPerBeat);
      while (
          relevantMelodyIndex.toDouble() / relevantMelody.subdivisionsPerBeat >=
              elementPosition.toDouble() / melody.subdivisionsPerBeat) {
        if (relevantMelodyIndex % relevantMelody.subdivisionsPerBeat ==
                0 && // Beginning of measure, stop searching
            (relevantMelodyIndex / relevantMelody.subdivisionsPerBeat) %
                    meter.defaultBeatsPerMeasure ==
                0) {
          break;
        }
      ...
      while (melodyResult == null) {
        ...
        if (relevantMelodyIndex.toDouble() /
                relevantMelody.subdivisionsPerBeat ==
            elementPosition.toDouble() / melody.subdivisionsPerBeat) {
          relevantMelodyIndex--;
          continue;
        }
        List<int> tones = relevantMelody
            .tonesAt(relevantMelodyIndex % relevantMelody.length)
            .toList();
        if (tones.isNotEmpty) {
          Iterable<NoteSign> matchingNotes = getPlaybackNotes(tones, chord)
              .where((relevantNote) =>
                  relevantNote.letter == note.letter &&
                  relevantNote.octave == note.octave)
              .map((relevantNote) => relevantNote.sign)
              .toSet();
          if (matchingNotes.isNotEmpty) {
            final key = relevantMelodyIndex.toDouble() /
                relevantMelody.subdivisionsPerBeat;
            resultCandidates
                .putIfAbsent(key, () => Set())
                .addAll(matchingNotes);
            break;
          }
        }
        relevantMelodyIndex--;
  ...
  static Path _flatPath = parseSvgPathData(
...
//        canvas.translate(-84.19600,-436.0680 + 1);
  ...
  _planLedgerLines(NoteSpecification note, List<double> result) {
    try {
      if (!clefs.any((clef) => clef.covers(note))) {
        Clef nearestClef = clefs.minBy((it) => min(
            (note.diatonicValue - it.diatonicMax).abs(),
            (note.diatonicValue - it.diatonicMin).abs()));
  ...
  List<NoteSpecification> getPlaybackNotes(List<int> tones, Chord chord) {
    ...
    if (computationChord.chroma == 2047 && true) {
      int newChroma = 0;
      newChroma =
          _addTonesToChroma(newChroma, computationChord.rootNote.tone, tones);
...
//      }
      ...
      if (useTonesInMeasure) {
        Iterable<int> tonesInMeasure =
            melody.tonesInMeasure(elementPosition, meter);
        newChroma = _addTonesToChroma(
            newChroma, computationChord.rootNote.tone, tonesInMeasure);
      }
      otherMelodiesOnStaff.forEach((otherMelody) {
        int otherPosition = elementPosition.convertPatternIndex(
            fromSubdivisionsPerBeat: melody.subdivisionsPerBeat,
            toSubdivisionsPerBeat: otherMelody.subdivisionsPerBeat);

        Iterable<int> otherTones =
            otherMelody.tonesInMeasure(otherPosition, meter);
        if (useTonesInMeasure) {
        ...
        newChroma = _addTonesToChroma(
            newChroma, computationChord.rootNote.tone, otherTones);
      });
      computationChord = computationChord.bsRebuild((Chord it) {
        it.chroma = newChroma;
      });
    }
  ...
  static int _addTonesToChroma(
      int chroma, int rootTone, Iterable<int> newTones) {
...
//  static List<List<dynamic>> _recentPlaybackNoteRequests =List();
  ...
  List<NoteSpecification> computePlaybackNotes(List<int> tones, Chord chord) =>
      tones.map<NoteSpecification>((int tone) {
  ...
  _drawFilledNotehead(Canvas canvas, Rect rect) {
    ...
    target = Rect.fromCenter(
        ...
        width: target.width,
        height: target.height * 0.7777777);
