...
class NoteSpecification {
  ...
  String get uiString =>
      "${noteName.noteLetter.name}${noteName.noteSign.simpleString}$octave";
  @override
  ...
  @override
  bool operator ==(other) =>
      other is NoteSpecification &&
      noteName == other.noteName &&
      octave == other.octave;
  @override
  int get hashCode => noteName.hashCode ^ octave.hashCode;
}

extension HeptatonicConversions on int {
  static final Map<int, List<NoteSpecification>> _notesFor = range(-4, 12)
      .map((octave) => NoteLetter.values.map((letter) => NoteSign.values.map(
          (sign) => NoteSpecification.name(
              letter: letter, sign: sign, octave: octave))))
      .expand((i) => i)
      .expand((i) => i)
      .toList()
  ...
  int playbackToneUnder(Chord chord, Melody melody) {
    if (melody.interpretationType !=
        MelodyInterpretationType.fixed_nonadaptive) {
...
class AccompanimentStaff extends MusicStaff {
  ...
  Iterable<Part> getParts(
      ...
      score.parts.where((part) => part.isHarmonic).where(
          (part) => // Don't display Parts listed elsewhere in the configuration.
              !staffConfiguration.any(
                  (staff) => staff is PartStaff && staff.part.id == part.id));
}
