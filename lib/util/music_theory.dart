...
extension NoteLetterTheory on NoteLetter {
  ...
  NoteLetter operator +(int increment) => NoteLetter.values
      .firstWhere((letter) => letter.value == (value + increment) % 7);
}
...
extension NoteConversions on int {
  bool get isBlackKey =>
      mod12 == 1 || mod12 == 3 || mod12 == 6 || mod12 == 8 || mod12 == 10;
  bool get isWhiteKey => !isBlackKey;
  int get mod12 => bsMod(12);
  int get mod7 => bsMod(7);
  int bsMod(int modulus) {
    int result = this;
...
extension ChordTheory on Chord {
  bool has(int halfStepsFromRoot) =>
      containsTone(rootNote.tone + halfStepsFromRoot);
  ...
  bool get hasDim5 =>
      has(6) &&
      !hasPer5 &&
      (hasMin3 || hasMaj3 || hasPer4 || (hasMin2 && hasMin7));
  bool get hasPer5 => has(7);
  bool get hasAug5 =>
      has(8) && !hasPer5 && (hasMaj6 || hasMaj3 || (!hasDim5 && hasMaj6));
...
extension MelodyTheory on Melody {
  ...
  double get averageTone =>
      averageToneCache.putIfAbsent(id, () => _averageTone);
  double get _averageTone => tones.length == 0
      ? 0
      : tones.reduce((a, b) => a + b) / tones.length.toDouble();
  ...
  Iterable<int> tonesInMeasure(int elementPosition, Meter meter) {
    ...
    return tonesInMeasureCache.putIfAbsent(
        key, () => _tonesInMeasure(elementPosition, meter));
  }
  ...
  Iterable<int> _tonesInMeasure(int elementPosition, Meter meter) {
    if (type == MelodyType.midi) {
      ...
      int measureStart = this.measureStart(elementPosition, meter);
      range(measureStart,
              measureStart + meter.defaultBeatsPerMeasure * subdivisionsPerBeat)
          .forEach((pos) {
...
extension Enabled on MelodyReference {
  bool get isEnabled =>
      this.playbackType.value != MelodyReference_PlaybackType.disabled.value;
}
...
//  String get convenientName => (name.isEmpty) ? name : "Part ${id.substring(0, 5)}";
  ...
  String get midiName =>
      isDrum ? "Drums" : midiInstruments[instrument.midiInstrument];
}
